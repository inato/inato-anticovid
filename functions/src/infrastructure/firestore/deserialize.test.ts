...
import { trialIdFactory, Subscription, subscriptionIdFactory, EmailAddress, searchFactory, facetFiltersFactory } from "../../domain";
import { deserialize } from "./deserialize";
import { GenericErrorType } from "../../domain/errors";
import { pipe } from "fp-ts/lib/pipeable";

const setupUseCase = ({ search }: Partial<{
  search: Partial<{
  ...
  it('should deserialize a firestore document to a Subscription', () => {
    ...
    expect(simplifiedUseCase()).toStrictEqual(Either.right(
      new Subscription({
        ...
        email: EmailAddress.unsafe_parse("user@inato.com"),
